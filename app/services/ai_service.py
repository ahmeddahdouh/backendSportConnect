...
from app.repositories.event_repository import EventRepository  # ← à adapter si besoin
...
def predict_category(question, threshold=0.4):
    ...
    if pred[idx] < threshold:
        return "Je ne suis pas sûr de comprendre. Peux-tu reformuler ?", None
    answer = unique_answers[idx]
    sport = extract_sport_de_answer(answer)
    descrip,events=get_events_description(sport)
   
    return answer,descrip
...
def get_events_description(question):
    sport = extract_sport_de_answer(question)
    if sport:
        events = event.get_events_by_sport(sport)
        description = event.get_events_description_by_sport(sport)
